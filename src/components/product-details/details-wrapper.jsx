...
const DetailsWrapper = ({ productItem, handleImageActive, activeImg, detailsBottom = false }) => {
  const { sku, img, title, imageURLs, category, description, discount, price, status, reviews, tags, offerExpiryTime } = productItem || {};
  ...
  return (
    <div className="tp-product-details-wrapper">
      ...
      {/* variations */}
      {imageURLs[0]?.color && <div className="tp-product-details-variation">
          ...
          <div className="tp-product-details-variation-list">
            {imageURLs.map((item, i) => (
              <button onClick={() => handleImageActive(item)} key={i} type="button"
                ...
                <span data-bg-color={`${item.color.clrCode}`}
                  style={{ backgroundColor: `${item.color.clrCode}` }}></span>
